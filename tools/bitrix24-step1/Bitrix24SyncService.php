...
final class Bitrix24SyncService
{
    ...
    public function sync(
        ...
        $warnings = [];
        $rawTasks = $this->client->tasks(
            (int) $link['bitrix_group_id'],
            (string) $link['report_tag'],
            200
        );

        if (count($rawTasks) >= 200) {
            $warnings[] = 'Получены первые 200 задач. Для крупных проектов потребуется фоновая постраничная синхронизация.';
        }

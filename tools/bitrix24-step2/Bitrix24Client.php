...
final class Bitrix24Client
{
    ...
    public function projects(): array
    {
        ...
        } while ($rows !== [] && count($result) < $total && $start < 5000);
    ...
    public function companies(): array
    {
        ...
        do {
            $response = $this->call('crm.item.list', [
                'entityTypeId' => 4,
                'select' => ['id', 'title', 'assignedById', 'createdTime', 'updatedTime'],
                'order' => ['title' => 'ASC'],
                'start' => $start,
            ]);
    ...
    /**
     * Загружает задачи только по проекту.
     * Фильтрация по тегу выполняется локально в Bitrix24SyncService,
     * потому что TAG не является надёжным полем фильтра tasks.task.list.
     */
    ...
    public function elapsedItems(
        ...
        do {
            ...
            $total = (int) ($response['total'] ?? count($result));
            $page++;
        } while ($rows !== [] && count($result) < $total && $page <= 100);
    ...
    {
        $url = $this->webhookBase . rawurlencode($method) . '.json';
        $payload = json_encode(
            $params,
        ...
        if (!is_string($payload)) {
            throw new RuntimeException('Не удалось подготовить запрос Битрикс24.');
        }

        $ch = curl_init($url);

        if ($ch === false) {
            throw new RuntimeException('Не удалось инициализировать запрос Битрикс24.');
        }
        ...
        if (!is_array($decoded)) {
            throw new RuntimeException('Битрикс24 вернул некорректный ответ.');
        }
        ...
        if ($httpCode >= 400 || isset($decoded['error'])) {
            ...
            throw new RuntimeException("Битрикс24 {$method}: {$description}");
        }
    ...
    {
        $host = parse_url($this->webhookBase, PHP_URL_HOST);
    ...
    private function loadWebhook(): string
    {
        ...
        throw new RuntimeException('Интеграция Битрикс24 не настроена.');
    }
    ...
    private function normalizeWebhook(string $url): string
    {
        ...
        if (!preg_match(
            ...
            throw new RuntimeException('Некорректный URL входящего вебхука Битрикс24.');
        }

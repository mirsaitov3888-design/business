...
function r24download(string $url, string $sha): string
{
    $context = stream_context_create([
        'http' => [
            'timeout' => 120,
            'follow_location' => 1,
            'user_agent' => 'Mirsaitov Cumulative Update/2026.08.02.4',
...
$alreadyInstalled = str_contains($service, 'SYSTEM_UPDATE_MANIFEST_NO_CACHE_V180204')
    && str_contains($js, 'SYSTEM_UPDATE_HISTORY_SELECTION_V180204')
...
$backupDirectory = $root . '/storage/backups/update-cache-navigation-' . date('Ymd-His');
...
try {
    if (!str_contains($service, 'SYSTEM_UPDATE_MANIFEST_NO_CACHE_V180204')) {
        ...
        $headerNeedle = "            CURLOPT_ENCODING => '',\n";
        $headerReplacement = "            CURLOPT_ENCODING => '',\n"
            . "            CURLOPT_HTTPHEADER => [\n"
            . "                'Cache-Control: no-cache, no-store, max-age=0',\n"
            . "                'Pragma: no-cache',\n"
            . "            ],\n";
        $service = str_replace($headerNeedle, $headerReplacement, $service, $headerCount);
        if ($headerCount !== 1) {
            throw new RuntimeException('Не удалось добавить HTTP-заголовки запрета кэширования.');
        }
    }
    ...
    if (!str_contains($js, 'SYSTEM_UPDATE_HISTORY_SELECTION_V180204')) {
        $needle = "        const row = rows[0];\n";
        $replacement = "        /* SYSTEM_UPDATE_HISTORY_SELECTION_V180204 */\n"
            . "        const activeRow = rows.find(item => [\n"
            . "            'queued', 'installing', 'rollback_queued', 'rolling_back'\n"
            . "        ].includes(item.status));\n"
            . "        const successfulRow = rows.find(item => [\n"
            . "            'installed', 'rolled_back'\n"
            . "        ].includes(item.status));\n"
            . "        const row = activeRow || successfulRow || rows[0];\n";
        $js = str_replace($needle, $replacement, $js, $selectionCount);
        if ($selectionCount < 1) {
            throw new RuntimeException('Не удалось изменить выбор последней операции.');
        }
    }
...
JSPATCH;
    ...
    $index = preg_replace('#/assets/app\.js\?v=\d+#', '/assets/app.js?v=180204', $index) ?? $index;
    ...
    r24out('Кэш обновлений, старая карточка ошибки и дубли меню исправлены.');
    ...
    r24out('- старые ошибки остаются только в журнале операций;');
